...
public class CSVUtilTest {
    ...
    //filtro para sacar jugadores mayores a 34 años y que son del mismo equipo
    @Test
    void reactive_filtrarJugadoresMayoresA35conEquipo() {
        List<Player> list = CsvUtilFile.getPlayers();
        Flux<Player> listFlux = Flux.fromStream(list.parallelStream()).cache();
        Mono<Map<Integer, Collection<Player>>> listFilter = listFlux
                .filter(player -> player.age >= 34 && player.club.equals("Chelsea"))
                .distinct()
    ...
    //filtro parea traer nacionalidades de jugadores
    @Test
    void reactive_filtrarJugadoresporNacionalidad() {
       ...
       listFilter.block().forEach( (item , var) -> System.out.println(item) );
